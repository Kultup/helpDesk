  ...
  return (
    <>
      ...
      <div 
        ...
        className={`${isMobile ? 'fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 sm:absolute sm:left-auto sm:top-auto sm:translate-x-0 sm:translate-y-0 sm:right-0 sm:mt-2' : 'absolute right-0 mt-2'} ${isMobile ? 'w-[calc(100vw-1rem)] sm:w-80 max-w-sm' : isTablet ? 'w-96' : 'w-80'} bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700 z-50 ${isMobile ? 'max-h-[calc(100vh-5rem)]' : 'max-h-96'} overflow-hidden touch-manipulation`}
      >
      {/* Header */}
        ...
        {notificationTickets.length === 0 ? (
          ...
          notificationTickets.map((ticket) => (
            <Link
              ...
              onClick={onClose}
              className={`block px-3 sm:px-4 py-3 sm:py-3 border-l-4 ${getPriorityColor(ticket.priority)} hover:bg-gray-50 dark:hover:bg-gray-700/50 active:bg-gray-100 dark:active:bg-gray-700 transition-colors touch-manipulation min-h-[60px] sm:min-h-0 bg-white dark:bg-gray-800/50`}
            >
              <div className="flex items-start space-x-2 sm:space-x-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-700 dark:text-gray-300 mb-2 line-clamp-2 break-words">
                    {ticket.description}
                  </p>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-0">
                    ...
                    {ticket.assignedTo && (
                      <div className="flex items-center text-xs text-gray-600 dark:text-gray-400">
                        <User className="h-3 w-3 mr-1 flex-shrink-0 text-gray-600 dark:text-gray-400" />
                        <span className="truncate max-w-[120px] sm:max-w-20 text-gray-600 dark:text-gray-400">
