...
class ApiService {
  ...
  constructor() {
    ...
    if (process.env.NODE_ENV === 'development') {
      ...
      console.log(
        '[API SERVICE] Development режим: використовується відносний шлях /api (проксується через setupProxy)'
      );
    } else if (!process.env.REACT_APP_API_URL) {
      // eslint-disable-next-line no-console
      console.warn(
        '[API SERVICE] REACT_APP_API_URL не встановлено, використовується відносний шлях /api'
      );
    }
  ...
  async login(credentials: LoginForm): Promise<ApiResponse<{ user: User; token: string }>> {
    const response: AxiosResponse<ApiResponse<{ user: User; token: string }>> = await this.api.post(
      '/auth/login',
      credentials
    );
    return response.data;
  ...
  async getTickets(
    ...
    const response: AxiosResponse<TicketsApiResponse> = await this.api.get(
  ...
  async createTicket(ticket: CreateTicketForm, files?: File[]): Promise<ApiResponse<Ticket>> {
    ...
    try {
      ...
      if (files && files.length > 0) {
        ...
        const response: AxiosResponse<ApiResponse<Ticket>> = await this.api.post(
          '/tickets',
          formData
        );
        // eslint-disable-next-line no-console
  ...
  async updateTicket(id: string, updates: UpdateTicketForm): Promise<UpdateTicketResponse> {
    const response: AxiosResponse<UpdateTicketResponse> = await this.api.put(
      `/tickets/${id}`,
      updates
    );
    return response.data;
  ...
  async analyzeTicket(id: string): Promise<
    ApiResponse<{
      summary: string;
      rootCause: string;
  ...
  async bulkDeleteTickets(ticketIds: string[]): Promise<ApiResponse<{ deletedCount: number }>> {
    const response: AxiosResponse<ApiResponse<{ deletedCount: number }>> = await this.api.delete(
      '/tickets/bulk/delete',
      { data: { ticketIds } }
    );
    return response.data;
  ...
  async sendTelegramMessage(
    ticketId: string,
    message: string
  ): Promise<ApiResponse<{ ticketId: string; sentAt: string }>> {
  ...
  async getCities(params?: {
    ...
    sortBy?: string;
    sortOrder?: 'asc' | 'desc';
  }): Promise<ApiResponse<City[]> & { pagination?: Record<string, unknown> }> {
    ...
    const response: AxiosResponse<ApiResponse<City[]> & { pagination?: Record<string, unknown> }> =
      await this.api.get(url);
    return response.data;
  }

  async getSimpleCities(): Promise<
    ApiResponse<Array<{ id: string; name: string; region: string }>>
  > {
    const response: AxiosResponse<
      ApiResponse<Array<{ id: string; name: string; region: string }>>
    > = await this.api.get('/cities/simple/list');
    return response.data;
  }
  ...
  async updateCity(id: string, updates: Partial<City>): Promise<ApiResponse<City>> {
    const response: AxiosResponse<ApiResponse<City>> = await this.api.put(`/cities/${id}`, updates);
    return response.data;
  }
  ...
  async getSimplePositions(): Promise<
    ApiResponse<Array<{ id: string; title: string; department: string }>>
  > {
    const response: AxiosResponse<
      ApiResponse<Array<{ id: string; title: string; department: string }>>
    > = await this.api.get('/positions/simple/list');
    return response.data;
  }

  async createPosition(position: CreatePositionData): Promise<ApiResponse<Position>> {
    const response: AxiosResponse<ApiResponse<Position>> = await this.api.post(
      '/positions',
      position
    );
    return response.data;
  ...
  async updatePosition(id: string, updates: Partial<Position>): Promise<ApiResponse<Position>> {
    const response: AxiosResponse<ApiResponse<Position>> = await this.api.put(
      `/positions/${id}`,
      updates
    );
    return response.data;
  ...
  async activatePosition(id: string): Promise<ApiResponse<Position>> {
    const response: AxiosResponse<ApiResponse<Position>> = await this.api.patch(
  ...
  async getPositionRequests(params?: {
    ...
    const response: AxiosResponse<ApiResponse<any[]>> = await this.api.get(
      `/position-requests${queryParams.toString() ? `?${queryParams.toString()}` : ''}`
    );
    return response.data;
  }
  ...
  async updateCategory(id: string, updates: UpdateCategoryForm): Promise<ApiResponse<Category>> {
    const response: AxiosResponse<ApiResponse<Category>> = await this.api.put(
      `/categories/${id}`,
      updates
    );
    return response.data;
  ...
  async deleteCategory(id: string): Promise<ApiResponse<null>> {
    const response: AxiosResponse<ApiResponse<null>> = await this.api.delete(`/categories/${id}`);
    return response.data;
  }

  async deactivateCategory(id: string): Promise<ApiResponse<Category>> {
    const response: AxiosResponse<ApiResponse<Category>> = await this.api.patch(
  ...
  async getPendingRegistrations(params?: {
    ...
    const response: AxiosResponse<ApiResponse<User[]>> = await this.api.get(
      `/users/pending-registrations${queryParams.toString() ? `?${queryParams.toString()}` : ''}`
    );
    return response.data;
  }
  ...
  async getPendingRegistrationsCount(): Promise<ApiResponse<{ count: number }>> {
    const response: AxiosResponse<
      ApiResponse<{ docs: User[]; pagination: { totalItems: number } }>
  ...
  ): Promise<ApiResponse<{ updated: number; errors: Array<Record<string, unknown>> }>> {
    const response: AxiosResponse<
      ApiResponse<{ updated: number; errors: Array<Record<string, unknown>> }>
  ...
  async testADConnection(): Promise<ApiResponse<Record<string, unknown>>> {
    const response: AxiosResponse<ApiResponse<Record<string, unknown>>> =
      await this.api.get('/active-directory/test');
    return response.data;
  }
  ...
  async getADUsers(params?: {
    ...
    const url = queryParams.toString()
      ? `/active-directory/users?${queryParams.toString()}`
  ...
  async getADComputers(params?: {
    ...
    const url = queryParams.toString()
      ? `/active-directory/computers?${queryParams.toString()}`
  ...
  async getAnalytics(dateFrom?: string, dateTo?: string): Promise<ApiResponse<AnalyticsData>> {
    ...
    const response: AxiosResponse<ApiResponse<AnalyticsData>> = await this.api.get(url);
    return response.data;
  }
  ...
  async getKnowledgeBaseArticles(params?: {
    ...
    if (params?.tags != null) searchParams.set('tags', params.tags);
    if (params?.page != null) searchParams.set('page', String(params.page));
    if (params?.limit != null) searchParams.set('limit', String(params.limit));
    if (params?.sortBy != null) searchParams.set('sortBy', params.sortBy);
    if (params?.all != null) searchParams.set('all', params.all);
    const url = `/kb/articles${searchParams.toString() ? `?${searchParams.toString()}` : ''}`;
  ...
  async getKnowledgeBaseArticle(id: string): Promise<{ success: boolean; data: unknown }> {
    const response = await this.api.get(`/kb/articles/${id}`);
  ...
  async createKnowledgeBaseArticle(data: {
    ...
    const response = await this.api.post('/kb/articles', data);
  ...
  async updateKnowledgeBaseArticle(
    ...
    const response = await this.api.put(`/kb/articles/${id}`, data);
    return response.data as { success: boolean; data: unknown; message?: string };
  }

  async deleteKnowledgeBaseArticle(id: string): Promise<{ success: boolean; message?: string }> {
    const response = await this.api.delete(`/kb/articles/${id}`);
  ...
  async generateAIReport(
    startDate?: string,
    endDate?: string,
    reportType: 'general' | 'weekly' | 'monthly' | 'detailed' = 'general'
  ): Promise<
    ApiResponse<{
      report: string;
      generatedAt: string;
  ...
  > {
    const response = await this.api.post('/analytics/generate-report', {
      startDate,
      endDate,
      reportType,
    });
  ...
  // Методи для експорту
  async exportTickets(format: 'csv' | 'excel', filters?: TicketFilters): Promise<Blob> {
    const params = new URLSearchParams();
  ...
  async getTicketHistory(ticketId: string): Promise<ApiResponse<Array<Record<string, unknown>>>> {
    const response: AxiosResponse<ApiResponse<Array<Record<string, unknown>>>> = await this.api.get(
  ...
  async getTicketHistoryStats(ticketId: string): Promise<ApiResponse<Record<string, unknown>>> {
    const response: AxiosResponse<ApiResponse<Record<string, unknown>>> = await this.api.get(
      `/tickets/${ticketId}/history/stats`
    );
    return response.data;
  }
  ...
  async addHistoryEntry(
    ...
    const response: AxiosResponse<ApiResponse<Record<string, unknown>>> = await this.api.post(
      `/tickets/${ticketId}/history`,
      data
    );
    return response.data;
  ...
  async getEventById(id: string): Promise<ApiResponse<CalendarEvent>> {
    const response: AxiosResponse<ApiResponse<CalendarEvent>> = await this.api.get(`/events/${id}`);
    return response.data;
  }

  async createEvent(event: CreateEventForm): Promise<ApiResponse<CalendarEvent>> {
    const response: AxiosResponse<ApiResponse<CalendarEvent>> = await this.api.post(
      '/events',
      event
    );
    return response.data;
  ...
  async updateEvent(id: string, event: UpdateEventForm): Promise<ApiResponse<CalendarEvent>> {
    const response: AxiosResponse<ApiResponse<CalendarEvent>> = await this.api.put(
      `/events/${id}`,
      event
    );
    return response.data;
  ...
  async removeEventParticipant(
    ...
    const response: AxiosResponse<ApiResponse<CalendarEvent>> = await this.api.delete(
  ...
  async markEventCompleted(eventId: string): Promise<ApiResponse<CalendarEvent>> {
    const response: AxiosResponse<ApiResponse<CalendarEvent>> = await this.api.patch(
  ...
  async linkTelegram(telegramId: string): Promise<ApiResponse<User>> {
    const response: AxiosResponse<ApiResponse<User>> = await this.api.post('/telegram/link', {
      telegramId,
    });
    return response.data;
  }
  ...
  async unlinkTelegram(): Promise<ApiResponse<User>> {
    const response: AxiosResponse<ApiResponse<User>> = await this.api.delete('/telegram/link');
    return response.data;
  }
  ...
  async getNotificationTemplates(params?: {
    type?: 'telegram' | 'web' | 'sms';
    category?: 'ticket' | 'user' | 'system' | 'security' | 'maintenance';
  ...
  async createNotificationTemplate(
    data: CreateNotificationTemplateForm
  ): Promise<ApiResponse<NotificationTemplate>> {
    const response: AxiosResponse<ApiResponse<NotificationTemplate>> = await this.api.post(
  ...
  async getInstitutionTypes(): Promise<
    ApiResponse<Array<{ value: InstitutionType; label: string; labelEn: string }>>
  > {
  ...
  async getSimpleInstitutions(): Promise<
    ApiResponse<Array<{ id: string; name: string; type: InstitutionType; city: string }>>
  > {
  ...
  async updateActiveDirectorySettings(
    data: Record<string, unknown>
  ): Promise<ApiResponse<Record<string, unknown>>> {
  ...
  async createZabbixGroup(
    data: Record<string, unknown>
  ): Promise<ApiResponse<Record<string, unknown>>> {
    return this.post('/zabbix/groups', data);
  }

  async updateZabbixGroup(
    id: string,
    data: Record<string, unknown>
  ): Promise<ApiResponse<Record<string, unknown>>> {
