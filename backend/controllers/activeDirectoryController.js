...
const enableUser = async (req, res) => {
  try {
    ...
    if (!username || !/^[\w.-]{1,64}$/.test(username)) {
      return res.status(400).json({ success: false, message: "Невалідне ім'я користувача" });
    }
    const result = await activeDirectoryService.enableUser(username);
...
const disableUser = async (req, res) => {
  try {
    ...
    const result = await activeDirectoryService.disableUser(username);
    return res.json({ success: result.success, message: result.message });
  } catch (error) {
    return res.status(500).json({ success: false, message: error.message });
  }
