 ...
 * @access  Private (Admin only)
 */
router.post('/:id/approve', authenticateToken, requirePermission('manage_positions'), async (req, res) => {
  try {
    const { id } = req.params;
 ...
 * @access  Private (Admin only)
 */
router.post('/:id/reject', authenticateToken, requirePermission('manage_positions'), async (req, res) => {
  try {
    const { id } = req.params;
