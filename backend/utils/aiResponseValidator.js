...
class AIResponseValidator {
    constructor() {
        ...
        this.requiredPatterns = {
            steps: /\d+[.)]\s+/,
            actionVerbs: /(перевірте|спробуйте|зробіть|вимкніть|увімкніть|натисніть|відкрийте|закрийте|перезавантажте)/i
        };
    }
    ...
    hasExcessiveRepetition(text) {
        ...
        for (const word of words) {
            if (word.length < 4) continue; // Ігноруємо короткі слова
     ...
     * @param {string} userMessage - Оригінальне повідомлення користувача
     * @returns {string} Fallback повідомлення
     */
    getFallbackQuickSolution(userMessage) {
        return "Розумію вашу проблему. " +
