...
analyticsSchema.virtual('isExpired').get(function () {
  if (!this.cacheSettings.ttl) {
    return false;
  }
  const expiryTime = new Date(
    this.cacheSettings.lastUpdated.getTime() + this.cacheSettings.ttl * 1000
  );
...
analyticsSchema.virtual('resolutionRate').get(function () {
  if (this.metrics.totalTickets === 0) {
    return 0;
  }
...
analyticsSchema.statics.findByScope = function (scope, type = null) {
  ...
  if (scope.city) {
    query['scope.city'] = scope.city;
  }
  if (scope.user) {
    query['scope.user'] = scope.user;
  }
  if (scope.department) {
    query['scope.department'] = scope.department;
  }
  if (scope.position) {
    query['scope.position'] = scope.position;
  }
...
analyticsSchema.methods.addForecast = function (
  metric,
  period,
  predictedValue,
  confidence,
