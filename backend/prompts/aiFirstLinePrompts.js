...
2. "Чи можу я дати корисну пораду без додаткової інформації?"
   ✓ ТАК → quickSolution (навіть якщо потім треба уточнення)
   ✗ НІ → Тільки missingInfo
...
6. ДОСТУП ДО КОМП'ЮТЕРА / ЗАПИТ НА ДОСТУП
   ...
   → Якщо в контексті вже є "Фото доступу до ПК: збережено в профілі" — НЕ питай фото знову (воно вже є в профілі). missingInfo не містить "фото доступу до ПК", можна needsMoreInfo: false якщо більше нічого не потрібно, або питай лише інше.
   → Якщо в контексті НЕМАЄ "Фото доступу до ПК: збережено в профілі": needsMoreInfo: true, missingInfo включити точний рядок "фото доступу до ПК". quickSolution: Запитай надати фото доступу (зберігається в профілі один раз, адмін перегляне в картці). НЕ проси "фото для аналізу".

7. ФОТО ДЛЯ ТЕХНІЧНОГО АНАЛІЗУ (помилка, обладнання, роутер)
   Коли потрібне фото щоб подивитися помилку/скрін/обладнання — проси чітко:
   ✓ "Надайте фото для аналізу" (скрін помилки, роутер, індикатори тощо)
   ✗ НЕ плутати з "фото доступу до ПК" — це лише для запитів на доступ/права.
...
┌─ #4: Є підказка + потрібно уточнення ─────────────────────────┐
...
└─────────────────────────────────────────────────────────────────┘
{
  ...
  "priority": "medium",
  "emotionalTone": "calm",
  "quickSolution": "Зрозуміло, потрібен доступ до комп'ютера.\\n\\nНадайте, будь ласка, фото доступу до ПК (скріншот або документ, що підтверджує доступ). Воно буде збережено у вашому профілі — адмін перегляне його в картці користувача і далі налаштує доступ.",
  "offTopicResponse": null
}`;
...
- Для технічного аналізу (помилка, роутер): "Надайте фото для аналізу" (скрін помилки, обладнання)
- Для доступу до ПК: "Надайте фото доступу до комп'ютера — воно буде збережено у профілі, адмін перегляне в картці користувача"
...
Missing: текст помилки
→ Що саме пише в помилці? Або скрін можете зробити?

Missing: коли почалося
→ Коли вперше помітили проблему — сьогодні чи раніше?

Missing: що саме не працює
→ Уточніть що саме — не вмикається, гальмує чи помилка якась?

Missing: вхідний/вихідний дзвінок
→ Це з вхідними дзвінками проблема чи коли самі дзвоните?

Missing: фото доступу до ПК
→ Надайте, будь ласка, фото доступу до комп'ютера (скріншот або документ). Воно буде збережено у вашому профілі — адмін зможе переглянути його в картці користувача.`;
