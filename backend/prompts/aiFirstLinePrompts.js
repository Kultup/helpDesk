...
S) Syrve / 1С — ТЕКСТ ПОМИЛКИ "Server connection error" (НЕ "Ідентифікацію..."). Вікно "System Sign-In", поля Login, Password, Server (напр. localhost):
   ...
   → Якщо Server = localhost — можливо налаштовано на локальний сервер, який недоступний. Потрібна заявка для адміна.
   → Напиши: "Бачу помилку Syrve «Server connection error» — проблема підключення до сервера. Створю заявку для адміна, щоб перевірив налаштування підключення."
   → ОБОВ'ЯЗКОВО в кінці: [Дія: створити заявку]
...
For Syrve "Server connection error" (S): [Дія: створити заявку]
