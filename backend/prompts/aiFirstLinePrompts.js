...
A) "Не можу прослуховувати дзвінки" / "не прослуховуються дзвінки" = одна проблема (запис/прослуховування).
   → Required: який номер (номер телефону/лінії), яке місто/філіал — для маршрутизації заявки.
   → missingInfo if not in message: ["номер телефону (який саме)", "місто або філіал"] + optionally "коли виникла проблема".
   → If user ALREADY wrote number (+380..., 0...) or city (Львів, філіал) — do NOT ask again. Only ask what is still missing.
...
┌─ TELEPHONY: номер і локація вже в повідомленні ──────────────────┐
│ "З цього робочого номера не можемо прослуховувати дзвінки, +380500646748 Львів Антоновича" │
└─────────────────────────────────────────────────────────────────┘
User already provided phone number and location. Do NOT ask for "місто" or "номер телефону" again. Only ask for what is still missing (e.g. when the problem started).
{
  "isTicketIntent": true,
  "needsMoreInfo": true,
  "category": "Phone",
  "missingInfo": ["коли вперше виникла проблема з дзвінками"],
  "confidence": 0.85,
  "priority": "medium",
  "emotionalTone": "calm",
  "quickSolution": "Зрозуміло, проблема з прослуховуванням дзвінків. Номер та локацію бачу, дякую 👍\n\nПідкажіть, будь ласка, коли вперше виникла проблема з дзвінками? Як тільки отримаю це — створю заявку для перевірки (Рінгостат/телефонія).",
  "offTopicResponse": null
}
...
Telephony (Phone), прослуховування, number and city already in message, missing: when started
→ Коли вперше виникла проблема з дзвінками?
