...
class MetricsCollector {
  ...
  getStats() {
    ...
    const totalTickets =
      this.metrics.ticketsCreated.aiMode + this.metrics.ticketsCreated.classicMode;
    const aiModePercentage =
      totalTickets > 0
    ...
    return {
      ...
      quickSolutionSuccessRate: total > 0 ? ((helpful / total) * 100).toFixed(1) + '%' : 'N/A',
      quickSolutionStats: {
        helpful,
        notHelpful,
        total,
      ...
      averageTicketCreationTime: this.calculateAverage(this.metrics.ticketCreationTimes),
      validationFailures: this.metrics.validationFailures,
      totalValidationFailures: Object.values(this.metrics.validationFailures).reduce(
        (a, b) => a + b,
        0
      ),
...
========================
  ...
  startDailyReset() {
    ...
    logger.info(
      '📊 Metrics collector initialized. Next reset in ' +
