...
`;
  ...
  async analyzeIntent(userMessage) {
    try {
      ...
      const systemPrompt = `
        ...
        - Якщо є БАЗОВА інформація (що зламалось + хоча б 1-2 деталі) → needsMoreInfo = false
        - Питайте додаткове ТІЛЬКИ якщо інформації КРИТИЧНО мало (наприклад: "не працює" без жодних деталей)
        - КРАЩЕ створити тікет з мінімальною інформацією, ніж задавати 10 питань користувачу
        ...
        ✅ ДОСТАТНЬО (needsMoreInfo = false):
        - "Не працює телефон Samsung" ->
           title: "Не працює телефон Samsung",
           description: "Телефон Samsung перестав працювати",
           needsMoreInfo: false (є модель + проблема = ДОСТАТНЬО!)
           
        - "Комп'ютер гальмує" ->
           title: "Комп'ютер гальмує",
           description: "Комп'ютер працює повільно",
           needsMoreInfo: false (зрозуміла проблема = ДОСТАТНЬО!)
        
        - "Не друкує принтер в бухгалтерії" ->
           title: "Не друкує принтер в бухгалтерії",
           description: "Принтер в бухгалтерії перестав друкувати",
           needsMoreInfo: false (місце + проблема = ДОСТАТНЬО!)
        ...
        💡 ПРАВИЛО: Якщо можна зрозуміти ЩО зламалось - цього ДОСТАТНЬО для тікета!
      `;
   ...
   * @param {number} currentRound - Поточний раунд питань (1, 2, 3...)
   * @param {number} maxRounds - Максимальна кількість раундів
   * @returns {Promise<string>} - Форматоване повідомлення з питаннями
   */
  async generateClarifyingQuestions(title, missingInfo = [], category = null, currentRound = 1, maxRounds = 3) {
    try {
      if (!this.client) {
        await this.initialize();
      }
      ...
      // Визначаємо тон залежно від раунду
      const isLastRound = currentRound >= maxRounds;
      const roundInfo = `[Раунд ${currentRound}/${maxRounds}]`;
      
      const systemPrompt = `
Ви - асистент технічної підтримки. Створіть КОРОТКІ, КОНКРЕТНІ уточнюючі питання для користувача.

${isLastRound ? 
`⚠️ ЦЕ ОСТАННІЙ РАУНД ПИТАНЬ! 
- Питайте ТІЛЬКИ найважливіше
- Будьте менш вимогливі до деталей
- Максимум 2-3 найкритичніших питання` 
: 
`ПРАВИЛА:
- ${currentRound === 1 ? '2-3 найважливіших питання' : '1-2 уточнюючих питання'}
- Кожне питання має бути коротким і конкретним`}

ОБОВ'ЯЗКОВО:
- Використовуйте емодзі для наочності
- МОВА: Українська
- Формат: список з bullet points

${isLastRound ? 
`Приклад (останній раунд):
📝 Останнє питання:
• Що саме написано в помилці?

💡 Якщо не знаєте точно - не страшно, створимо тікет.` 
: 
`Приклад (раунд ${currentRound}):
📝 Уточніть, будь ласка:
• Що саме не працює?
• Коли це почалося?
...
Що треба уточнити: ${missingInfo.join(', ')}
Раунд: ${currentRound}/${maxRounds}

${isLastRound ? 
'Останній раунд - питай ТІЛЬКИ найважливіше (1-2 питання)!' : 
`Створи ${currentRound === 1 ? '2-3' : '1-2'} конкретних питання.`}
`;
      ...
      const response = chatCompletion.choices[0]?.message?.content || 
        `📝 ${isLastRound ? 'Останнє' : 'Уточніть'}, будь ласка:\n${missingInfo.slice(0, isLastRound ? 2 : 3).map(info => `• ${info}?`).join('\n')}\n\n💡 ${isLastRound ? 'Якщо не знаєте - не страшно.' : 'Це допоможе швидше вирішити проблему.'}`;
