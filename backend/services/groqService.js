...
`;
  ...
  async analyzeIntent(userMessage) {
    try {
      ...
      // Використовуємо кастомний промпт з БД або дефолтний з коду
...
3. Якщо користувач відповів "Canon" - це модель, питай ПРО ІНШЕ (місто, що не працює, індикатори)
...
2. ✅ Потім задайте ОДНЕ конкретне питання про ЩО НЕ ВІДОМО
   
3. ✅ Будьте природними:
   - НЕ: "📝 Уточніть, будь ласка: • Питання 1? • Питання 2?"
   - ТАК: "Зрозумів! А в якому місті цей принтер?"
...
СТРУКТУРА ВІДПОВІДІ:
[Реакція на попереднє] + [Одне НОВЕ питання про невідоме]
...
Ви: "Добре. А в якому місті знаходиться цей принтер?"

Користувач: "Київ"
Ви: "Ага. Що саме не працює - зовсім не друкує, чи погана якість друку?"

Користувач: "Не друкує, горить червоний індикатор"
Ви: "Зрозумів. А на екрані принтера щось написано або моргає?"

⚠️ НЕ ПИШІТЬ СПИСКИ ПИТАНЬ! ТІЛЬКИ ОДНЕ ПИТАННЯ!
⚠️ НЕ ПОВТОРЮЙТЕ питання якщо користувач вже відповів!
...
`;
      ...
      // Якщо кастомного немає - використовуємо дефолтний
      if (!systemPrompt) {
        systemPrompt = `
Ві - експерт-аналітик системи HelpDesk з можливістю НАВЧАННЯ НА РЕАЛЬНИХ ДАНИХ.
...
`;
      } // Закриття блоку if (!systemPrompt) для дефолтного промпту
      
      // Додаємо історичні дані в промпт (якщо використовуємо кастомний)
      if (customPrompt && historicalDataSection) {
        systemPrompt = `${historicalDataSection}\n\n${systemPrompt}`;
      }
