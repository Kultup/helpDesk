...
`;
  ...
  async analyzeIntent(userMessage) {
    try {
      ...
      const systemPrompt = `
        ...
        2. Опис (description) - ДЕТАЛЬНИЙ, НЕ може дублювати заголовок! Має містити що саме не працює, коли виникла проблема
        3. Якщо інформації недостатньо - description має бути null, а needsMoreInfo = true
        4. НЕ вигадуйте проблеми або деталі, яких немає в повідомленні
        5. МОВА: Якщо вхідний текст не українською, ПЕРЕКЛАДІТЬ на українську
        ...
        - "Не працює телефон, я дуже злий!" -> 
           ...
           description: "Телефон перестав працювати, користувач незадоволений",
           needsMoreInfo: true,
           missingInfo: ["яка саме несправність", "модель телефону", "коли виникла проблема"],
           category: "Hardware", 
           sentiment: "negative", 
           priority: "high"
        
        - "Не друкує принтер" ->
           title: "Не друкує принтер",
           description: null,
           needsMoreInfo: true,
           missingInfo: ["модель принтера", "що саме відбувається", "чи горять індикатори", "чи є помилки на екрані"],
           isTicketIntent: true
        
        - "HP LaserJet не друкує, на екрані помилка E3, індикатор червоний" ->
           title: "HP LaserJet не друкує",
  ...
  /**
   * Генерує уточнюючі питання для покращення опису тікета
   ...
   * @returns {Promise<string>} - Форматоване повідомлення з питаннями
   */
  async generateClarifyingQuestions(title, missingInfo = [], category = null) {
    try {
      if (!this.client) {
        await this.initialize();
      }

      if (!this.client || !missingInfo || missingInfo.length === 0) {
        return `📝 Розумію, що у вас проблема: "${title}"\n\nБудь ласка, опишіть детальніше:\n• Що саме не працює?\n• Коли виникла проблема?\n• Які дії ви вже спробували?`;
      }

      const systemPrompt = `
Ви - асистент технічної підтримки. Створіть КОРОТКІ, КОНКРЕТНІ уточнюючі питання для користувача.

ПРАВИЛА:
1. Максимум 3-4 питання
2. Кожне питання має бути коротким і зрозумілим
3. Використовуйте емодзі для наочності
4. Питання мають допомогти зрозуміти проблему
5. МОВА: Українська
...
Треба уточнити: ${missingInfo.join(', ')}

Створи 3-4 конкретних питання для користувача.
`;
      ...
      logger.error('Помилка генерації уточнюючих питань:', error);
      return `📝 Будь ласка, опишіть проблему детальніше:\n${missingInfo.map(info => `• ${info}`).join('\n')}`;
    }
  }
