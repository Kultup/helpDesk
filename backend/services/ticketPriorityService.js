...
class TicketPriorityService {
    constructor() {
        ...
        this.criticalKeywords = [
            'не працює', 'зламався', 'критично', 'терміново', 'аварія',
            'не можу працювати', 'блокує роботу', 'зависає', 'помилка',
            'не запускається', 'втрата даних', 'безпека', 'вірус'
        ];

        this.urgentKeywords = [
            'директор', 'керівник', 'важливо', 'нарада', 'презентація',
            'дедлайн', 'клієнт', 'звіт'
        ];
    }
    ...
    calculateWaitingTimeScore(createdAt) {
        ...
        if (hoursWaiting < 1) return 10;
        if (hoursWaiting < 4) return 30;
        if (hoursWaiting < 8) return 50;
        if (hoursWaiting < 24) return 70;
        if (hoursWaiting < 48) return 85;
        ...
        switch (sla.status) {
            ...
            case 'on_time':
                ...
                if (sla.remainingHours && sla.hours) {
                    ...
                    if (percentRemaining < 20) return 60;
                    if (percentRemaining < 50) return 40;
                }
                return 20;
            case 'not_started':
                return 0;
    ...
    calculateReopenScore(reopenCount) {
        if (!reopenCount || reopenCount === 0) return 0;
    ...
    async calculateUserHistoryScore(userId) {
        try {
            ...
            if (openTickets.length > 3) return -20; // Негативний score
            if (openTickets.length > 1) return -10;

            // Якщо це перший тікет користувача - підвищуємо пріоритет
            if (userTickets.length === 1) return 30;
    ...
    async updateTicketPriority(ticketId, forceUpdate = false) {
        try {
            ...
            logger.info(`✅ Оновлено пріоритет тікету ${ticket.ticketNumber}: ${oldPriority} → ${priorityData.suggestedPriority} (score: ${priorityData.score})`);
    ...
    async updateAllTicketPriorities() {
        try {
            ...
            logger.info(`✅ Оновлення завершено: ${updated} оновлено, ${skipped} пропущено, ${errors.length} помилок`);
