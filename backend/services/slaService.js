...
class SLAService {
  ...
  async resumeSLA(ticketId) {
    try {
      ...
      ticket.sla.deadline = this.calculateDeadline(resumedAt, remainingHours, useBusinessHours);
      ...
      logger.info(
        `▶️ SLA відновлено для тікету ${ticket.ticketNumber} (пауза: ${Math.round(pauseDuration * 100) / 100}h)`
      );
