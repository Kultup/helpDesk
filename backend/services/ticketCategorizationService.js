  ...
  "reasoning": "коротке пояснення чому обрано саме цю категорію"
    ...
    async categorizeTicket(title, description) {
        try {
            ...
            const validSubcategories = Object.keys(this.categoryTree[categorization.category].subcategories);
            ...
            logger.info(`✅ Категоризація: ${categorization.category} → ${categorization.subcategory} (${Math.round(categorization.confidence * 100)}%)`);
    ...
    fallbackCategorization(title, description) {
        ...
        const keywords = {
            Hardware: ['принтер', 'комп\'ютер', 'монітор', 'клавіатура', 'миша', 'обладнання', 'не вмикається', 'зламався'],
            Software: ['програма', 'office', 'word', 'excel', '1с', 'браузер', 'антивірус', 'windows', 'не запускається'],
            ...
            for (const ticket of tickets) {
                try {
                    ...
                    if (result.confidence >= 0.7) {
                        ...
                        logger.info(`✅ Рекатегоризовано тікет ${ticket.ticketNumber}: ${result.category} → ${result.subcategory}`);
                    }
