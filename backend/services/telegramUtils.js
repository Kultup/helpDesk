            ...
            try {
                ...
                file.on('finish', () => {
                    ...
                    logger.info('Файл успішно завантажено з Telegram через getFileStream', {
                        fileId, localPath, size: stats.size
                    });
                ...
                stream.on('error', (error) => {
                    file.close();
                    fs.unlink(localPath, () => { });
                    logger.error('Помилка потоку при завантаженні файлу з Telegram', { fileId, error: error.message });
                    reject(error);
                });
            } catch (error) {
                logger.error('Помилка отримання потоку файлу з Telegram', {
                    fileId, error: error.message, stack: error.stack
                });
                reject(error);
            }
        });
            ...
            https.get(url, (response) => {
                if (response.statusCode !== 200) {
                    ...
                    logger.error(`Помилка завантаження файлу з Telegram: статус ${response.statusCode}`, {
                        filePath, url, statusCode: response.statusCode
                    });
                ...
                file.on('finish', () => {
                    ...
                    if (contentLength > 0 && stats.size !== contentLength) {
                        logger.warn('Розмір завантаженого файлу не відповідає Content-Length', {
                            filePath, localPath, expected: contentLength, actual: stats.size
                        });
                    }

                    logger.info('Файл успішно завантажено з Telegram', {
                        filePath, localPath, size: stats.size, contentLength
                    });
                ...
                logger.error('Помилка HTTP запиту при завантаженні файлу з Telegram', {
                    filePath, url, error: error.message
                });
                reject(error);
            });
        });
