...
class TelegramTicketService {
        ...
        try {
            ...
            await this.sendMessage(chatId, message, {
                reply_markup: {
                    inline_keyboard: [
                        [
                            { text: TelegramUtils.truncateButtonText(`🔄 Повторити: ${ticket.title}`, 50), callback_data: `recreate_ticket_${ticket._id}` },
            ...
            await this.sendMessage(chatId, `❌ *Помилка завантаження деталей*\nНе вдалося завантажити дані тікету`);
        }
    }
    ...
    async handleCreateTicketCallback(chatId, user) {
        ...
        await this.sendMessage(chatId,
            ...
            `💡 Опишіть коротко суть проблеми`, {
            reply_markup: {
                inline_keyboard: [[{ text: TelegramUtils.getCancelButtonText(), callback_data: 'cancel_ticket' }]]
            }
        });
    }
    ...
    async handleTicketPhoto(chatId, photos, caption) {
        try {
            ...
            let savedPath;
            try {
                savedPath = await TelegramUtils.downloadTelegramFileByFileId(this.bot, fileId, fileExtension);
    ...
    async completeTicketCreation(chatId, user, session) {
        try {
            ...
            try {
                ...
                await this.telegramService.notificationService.sendNewTicketNotificationToGroup(ticket, user);
            ...
            if (session.aiDialogId) {
                await this.telegramService.completeAIDialog(session.aiDialogId, 'ticket_created', ticket._id);
            }
