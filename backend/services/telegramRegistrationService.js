    ...
    async sendCitySelection(chatId, _userId) {
        try {
            ...
            await this.sendMessage(chatId,
                `✅ *Пароль прийнято!*\n` +
                `🔐 \`********\`\n` +
                `\n🏙️ *Крок 7/9:</b> Оберіть ваше місто\n` +
                `💡 Міста з іконкою 🏢 мають доступні заклади`,
                {
                    reply_markup: {
                        inline_keyboard: cityKeyboard
                    },
                    parse_mode: 'HTML' // Ensure HTML mode for consistency with previous step message
                }
            );
        } catch (error) {
    ...
    async handlePositionRequestCallback(callbackQuery) {
        try {
            ...
            if (!user || user.role !== 'admin') {
                await this.telegramService.answerCallbackQuery(callbackQuery.id, 'Тільки адміністратори можуть обробляти запити на посади');
                return;
            }
            ...
            if (data.startsWith('approve_position_')) {
                ...
                await this.telegramService.notificationService.notifyUserAboutPositionApproval(positionRequest, createdPosition);
                ...
                // Відправляємо сповіщення користувачу
                await this.telegramService.notificationService.notifyUserAboutPositionRejection(positionRequest, positionRequest.rejectionReason);
