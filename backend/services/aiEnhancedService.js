...
Якщо папір застрягає постійно — створю тікет для адміна 🔧`,
      ...
      'ringostat / телефонія': {
        keywords: [
          ...
          'не телефонують',
          'не дзвонять',
          'не можу зателефонувати',
          'телефон не працює',
          'не працює телефон',
          'дзвінки не проходять',
          'не чути',
          'не чутно',
          'переадресація',
          'вхідні не приходять',
          'вихідні не проходять',
          'smart phone',
        ],
        solution: `Спробуйте перед створенням заявки:
1️⃣ Перевірте інтернет (відкрийте будь-який сайт)
2️⃣ Відкрийте додаток <b>Ringostat Smart Phone</b> і переконайтесь що статус "Online"
3️⃣ Якщо статус "Offline" — вийдіть з додатку і зайдіть знову

Якщо не допоможе — створю тікет для адміна 📞`,
        category: 'Hardware',
        estimatedTime: '2-5 хвилин',
      },

      'мікротік / мережеве обладнання': {
        keywords: [
          'мікротік',
          'mikrotik',
          'маршрутизатор',
        ...
        solution: `Це мережеве обладнання — потрібен доступ адміністратора (Winbox / SSH).

Створюю заявку для мережевого адміна. Він підключиться віддалено і діагностує.

Якщо є термінова інформація (який хост / тунель / що саме) — додайте в коментар до заявки.`,
        category: 'Network',
        estimatedTime: '15-30 хвилин',
      },

      'інтернет не працює': {
        keywords: ['інтернет', 'не працює', 'немає звʼязку', 'wifi', 'wi-fi', 'мережа не працює'],
        solution: `Перевіримо:
1️⃣ Це тільки у вас чи у колег теж немає інтернету?
2️⃣ Перевірте мережевий кабель (чи щільно підключений)
3️⃣ Перезавантажте свій комп'ютер

⚠️ Якщо проблема у кількох людей одразу — одразу створюю тікет для мережевого адміна, роутер чіпати не потрібно 🌐`,
        category: 'Network',
...
Якщо не допоможе — створю тікет, адмін перевірить систему 💻`,
      ...
      'word або excel не працює': {
        keywords: [
          ...
          'офіс',
          'office',
          'не відкривається word',
          'не відкривається excel',
          'word не запускається',
          'excel не запускається',
          'ворд вилітає',
          'ексель вилітає',
          'помилка word',
          'помилка excel',
        ...
        solution: `Спробуйте:
1️⃣ Перезавантажте комп'ютер і спробуйте ще раз
2️⃣ Якщо програма вилітає з помилкою — зробіть скріншот помилки
3️⃣ Спробуйте відкрити інший файл (можливо проблема у конкретному документі)

Якщо не допоможе — створю тікет, адмін перевірить ліцензію та переустановить Office 📄`,
        category: 'Software',
        estimatedTime: '5-10 хвилин',
      },

      'syrve / iiko не працює': {
        keywords: ['syrve', 'sirve', 'iiko', 'айко', 'сирв', 'ресторанна програма'],
        solution: `Syrve / iiko потребує адміністратора для діагностики (сервер, ліцензія, оновлення).

Створюю тікет — адмін перевірить підключення та ліцензію. Якщо є скріншот помилки — надішліть, це прискорить вирішення 📸`,
        category: 'Software',
        estimatedTime: '15-30 хвилин',
      },

      '1с ліцензія / активація': {
        keywords: [
          'ліцензія 1с',
          '1с ліцензія',
          'ліцензія 1c',
          ...
          'активація 1с',
          '1с активація',
          'ліцензія закінчилась 1с',
          'немає ліцензії 1с',
        ],
        solution: `Ліцензія 1С — це задача адміністратора (активація через сервер ліцензій).

Створюю тікет, адмін активує ліцензію. Якщо є скріншот помилки — надішліть 📸`,
        category: 'Software',
        estimatedTime: '15-30 хвилин',
      },

      '1с не запускається': {
        keywords: ['1с', '1c', 'не запускається 1с', '1с не відкривається', '1c не запускається'],
