...
Користувач може:
  • натиснути «Створити тікет» (callback create_ticket) — тоді бот питає «Опишіть проблему…»;
  • або одразу написати проблему текстом (наприклад: «Принтер не друкує») — тоді бот одразу запускає AI-флоу з цим повідомленням (без додаткового запиту).

Перевіряємо AISettings (БД) → AI увімкнено + ключ є?
   ├── Так → Запускаємо AI-режим (сесія mode=ai, перше повідомлення вже в діалозі або запит «Опишіть проблему…»)
   └── Ні  → Якщо було натискання create_ticket — класичний покроковий флоу; якщо був просто текст — головне меню
```
...
- **Стиль спілкування:** промпти в `aiFirstLinePrompts.js` задають дружній, ввічливий тон; уточнюючі питання (NEXT_QUESTION) варіюються, не повторюють однакові фрази, створюють враження спілкування з живою людиною. Відповіді на «це не тікет» теж варіюються (випадковий вибір з кількох формулювань).
...
- **Старт AI з першого повідомлення:** якщо зареєстрований користувач пише текст (без активної сесії) і AI увімкнено — створюється AI-сесія, повідомлення передається в `handleMessageInAiMode` (analyzeIntent → далі по логіці AI). Користувач може одразу написати проблему, не натискаючи «Створити тікет».
